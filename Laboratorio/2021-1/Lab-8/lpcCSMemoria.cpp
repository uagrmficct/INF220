...
int lpcCSMemoria::numero_ids(string id) {
	if (id.length() == 0)
		return 0;
	int cont = 0;
...
int lpcCSMemoria::new_espacio(string cadena) {
...
void lpcCSMemoria::delete_espacio(int dir) {
	int x = dir;
...
void lpcCSMemoria::poner_dato(int dir, string cadena_id, int valor) {
	int z = dir;
...
int lpcCSMemoria::obtener_dato(int dir, string cadena_id) {
	int z = dir;
	cadena_id = cadena_id.substr(2, cadena_id.length() - 2);
	// Elimina la flecha
	while (z != NULO) {
...
int lpcCSMemoria::espacio_disponible() {
	int x = libre;
	int c = 0;
	while (x != -1) {
		c++;
...
int lpcCSMemoria::espacio_ocupado() {
	int c = MAX - espacio_disponible();
	return c;
}

bool lpcCSMemoria::dir_libre(int dir) {
	int x = libre;
	bool c = false;
	while (x != -1 && c == false) {
		if (x == dir)
