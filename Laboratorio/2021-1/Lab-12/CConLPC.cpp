...
CConjunto::CConjunto(CSMemoria* m) {
	cant = 0;
	ptr_conj = NULO;
	mem = m;
	srand(time(NULL));
...
bool CConjunto::vacio() {
	return cant == 0; // ptr_conj == NULO;
}

bool CConjunto::pertenece(int e) {
	int pc = ptr_conj;
	while (pc != NULO) {
...
int CConjunto::cardinal() {
	return cant;
}

int CConjunto::ordinal(int e) {
	int cont = 0;
	int pc = ptr_conj;
	while (pc != NULO) {
		cont++;
